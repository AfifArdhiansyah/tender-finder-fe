        ...
        <Table className="h-full" headers={props.bcIndex == 2 ? headerKC : headers} datas={props.datas} usePagination={false}>
            {props.datas?.map((data,i)=>(
                <tr className="border-b-[1px]" key={"row-"+i}>
                    ...
                    <td className="px-2 py-2 text-sm">{Number(parseFloat(data.total_diserap?.toString() || "0"))?.toLocaleString('id-ID')}</td>
                    <td className="px-2 py-2 text-sm">{data.percent_mengajukan_kredit}</td>
                    <td className="px-2 py-2 text-sm">
                        <div className="flex justify-center">
                            <Button type="primary" size="medium" onClick={()=>toDetail("Kanwil "+data.kanwil_id, data.kanwil_id)}>Detail</Button>
                        </div>
                    </td>
                </tr>
            ))}
