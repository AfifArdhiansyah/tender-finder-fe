...
export default function SidebarListChild(props: SidebarListChildProps){
    ...
    return (
        <Link href={props.ref} className={"w-full px-4 py-2 flex gap-4 items-center rounded-[10px] max-lg:justify-center "+ switchActiveClass(props.isActive)} onClick={onListClicked}>
            <Image src={props.isActive? props.iconPath+"-active.png" : props.iconPath+".png"} alt="icon" width={20} height={20}/>
            <p className="max-lg:hidden">{props.label}</p>
        </Link>
    )
}